...
var initEmbla = (root, options) => {
  const viewport = root.matches('[data-embla="viewport"]') ? root : root.querySelector('[data-embla="viewport"]');
  if (!viewport) return;
  ...
  const buttons = viewport.parentElement?.querySelectorAll("[data-dir]") ?? [];
  buttons.forEach((btn) => {
...
var boot = () => {
  const viewports = document.querySelectorAll('[data-embla="viewport"]');
...
//# sourceMappingURL=embla-init.js.map
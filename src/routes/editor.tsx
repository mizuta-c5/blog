...
editor.post('/new', requireAuth, async (c) => {
    ...
    await c.env.DB.prepare('INSERT INTO posts (slug, title, content) VALUES (?, ?, ?)').bind(slug, title, content).run()
...
editor.get('/edit/:slug', requireAuth, async (c) => {
    ...
    const row = await c.env.DB.prepare('SELECT slug, title, content FROM posts WHERE slug=?').bind(slug).first()
    if(!row) {
        return c.notFound();
    }
    const r = row as any;
    return c.html(Layout('Edit Post', html`
        <h1>Edit Post</h1>
        <form method="post" actioin="/edit/${r.slug}">
            <input name="title" required value="${r.title}" />
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <textarea name="content" id="md" rows="18" required class="w-full p-3 font-mono border rounded-lg">${r.content}</textarea>
            <div id="preview" class="w-full p-3 bg-white border rounded-lg overflow-auto prose"></div><button type="submit" class="mt-3">Update</button>
            </div>
        </form>
        <script type="module" src="/editor.js"></script>
        `, { wide: true }),
...
editor.post('/edit/:slug', requireAuth, async (c) => {
    ...
    await c.env.DB.prepare('UPDATE posts SET title=?, content=? WHERE slug=?').bind(title, content, slug).run()
    return c.redirect(`/p/${slug}`, 302)
})
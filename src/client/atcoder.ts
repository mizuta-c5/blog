...
function toWeeklySeries(raw: any[]) {
  ...
  const JST = 9 * 3600 * 1000, WEEK = 7 * 24 * 3600 * 1000
  ...
  const start = toMonJST(pts[0].t), end = new Date()
  const labels: string[] = [], values: number[] = []
  let w = start, i = 0, last = pts[0].r
  while (w <= end) {
    while (i < pts.length && pts[i].t < new Date(w.getTime() + WEEK)) { last = pts[i].r; i++ }
...
async function getWeeklyData(user: string) {
  ...
  try {
    const c = JSON.parse(localStorage.getItem(KEY) ?? 'null') as { savedAt: number; data: any[] } | null
    if (c && Date.now() - c.savedAt < ONE_WEEK) return c.data
  } catch {}
  const data = await (await fetch(`/atcoder/${encodeURIComponent(user)}`)).json() as any[]
